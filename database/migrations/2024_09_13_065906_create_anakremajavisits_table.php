...
return new class extends Migration
{
    ...
    {
        Schema::create('anakremajavisits', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('timbang_remaja',['Y','T']);
            $table->float('bb',2)->nullable();
            $table->float('tb',2)->nullable();
            $table->enum('waktu_timbang_remaja',['Y','T']);
            ...
            $table->string('jenis_pemeriksaan_gd')->nullable();
            ...
            // Skrining TBC
            $table->enum("remaja_batuk_terus",['Y','T']);
            $table->enum("remaja_demamLebih2minggu",['Y','T']);
            $table->enum("bb_remaja_tidaknaik2bulan",['Y','T']);
            $table->enum("remaja_kontak_pasientb",['Y','T']);
    ...
    {
        Schema::dropIfExists('anakremajavisits');
    }
};
