...
return new class extends Migration
{
    ...
    {
        Schema::create('lansiavisits', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('timbang_lansia',['Y','T']);
            $table->float('bb',2)->nullable();
            $table->float('tb',2)->nullable();
            $table->enum('waktu_timbang_lansia',['Y','T']);
            ...
            //Skrining AKS
            $table->enum('skrining_aks',['Y','T']);
            $table->string('waktu_pemeriksaan_skrining_aks')->nullable();
            $table->string('jenis_fasyankes_tempat_aks')->nullable();
            $table->string('nama_fasyankes_tempat_aks')->nullable();
            $table->enum('dokter_aks',['Y','T']);
            $table->enum('bidan_aks',['Y','T']);
            $table->enum('kader_aks',['Y','T']);
            $table->enum('petugas_gizi_aks',['Y','T']);
             //Skrining KILA
             $table->enum('skrining_kila',['Y','T']);
             $table->string('waktu_pemeriksaan_skrining_kila')->nullable();
             $table->string('jenis_fasyankes_tempat_kila')->nullable();
             $table->string('nama_fasyankes_tempat_kila')->nullable();
             $table->enum('dokter_kila',['Y','T']);
             $table->enum('bidan_kila',['Y','T']);
             $table->enum('kader_kila',['Y','T']);
             $table->enum('petugas_gizi_kila',['Y','T']);
            //Skrining Jiwa
            ...
            // Skrining TBC
            $table->enum("lansia_batuk_terus",['Y','T']);
            $table->enum("lansia_demamLebih2minggu",['Y','T']);
            $table->enum("bb_lansia_tidaknaik2bulan",['Y','T']);
            $table->enum("lansia_kontak_pasientb",['Y','T']);
    ...
    {
        Schema::dropIfExists('lansiavisits');
    }
};
